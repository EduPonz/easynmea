...
def get_project_name_large(cmakelist):
    """
    Get the large project name from a file

    The function looks for 'set(PROJECT_NAME_LARGE <name>)'

    :param cmakelists: The file to scan for the name
    :return: A string containing the name
    """
    matches = re.findall(
        'set\(PROJECT_NAME_LARGE\s+"[a-zA-Z ]+"\)',
        ''.join(open(cmakelist, 'r').readlines()),
        flags=re.MULTILINE)[0]
    return matches.split('"')[-2]
...
def configure_doxyfile(
    ...
    filedata = filedata.replace('@PROJECT_NAME_LARGE@', project_name)
    ...
    filedata = filedata.replace('@INDEX_FILE@', index_file)
    filedata = filedata.replace('@LICENSE_FILE@', license_file)
...
doxyfile_in = os.path.abspath(
    '{}/Doxyfile.in'.format(script_path)
)
doxyfile_out = os.path.abspath(
    '{}/Doxyfile'.format(project_binary_docs_dir)
)

# Header files
input_dir = os.path.abspath('{}/include'.format(project_source_dir))

# Other files
index_file = '{}/README.md'.format(project_source_dir)
license_file = '{}/LICENSE.md'.format(project_source_dir)
...
copyright = u'2021, Eduardo Ponz'
author = u'Eduardo Ponz'
...
htmlhelp_basename = '{} Manual'.format(project)
...
texinfo_documents = [
    (master_doc,
     ...
     author,
     '{}',
     'Documentation of {}'.format(project),
     'Miscellaneous'),
]
