...
To satisfy these requirements *EasyNMEA* uses `Gtest <https://google.github.io/googletest/>`_ as testing
framework.
...
* `Catch <https://github.com/catchorg/Catch2/tree/devel/docs>`_ seemed very promising, specially being a header only
  library, but the lack of mocking support is unfortunately a no-go for *EasyNMEA*.
...
3. :class:`BUILD_TESTS`: Builds all the *EasyNMEA* tests, meaning both library and documentation tests.
...
All the *EasyNMEA* tests run automatically once a day for the :class:`main` branch, as well as for the supported
versions' branches.
...
As stated in :ref:`dev_docs_testing_automated`, *EasyNMEA* tests are run with every push to :class:`main` and
...
`Codecov <https://app.codecov.io/gh/EduPonz/easynmea/>`_.
